...
const getAppliedUserNotApplied = {phoneNumber: 6043347758};
const getAppliedUserApplied = {phoneNumber: 6043388885};
const canApplyAlreadyAppliedUser = {userID: "dummy"};
const canApplyAlreadyAppliedJob = {jobID: "dummy"};
const canApplyIsCreatorUser = {userID: "dummy"};
const canApplyIsCreatorJob = {jobID: "dummy"};
const canApplyCorrectUser = {userID: "dummy"};
const canApplyCorrectJob = {jobID: "dummy"};
...
describe("Get Applied-For Jobs", function() { 
  ...
  it("Applicant has not applied for any jobs", function (done){
    let options = {phoneNumber: getAppliedUserNotApplied.phoneNumber};
    request(baseUrl + url, options,function(err, res, body) {
      expect(res.statusCode).to.equal(400);
      expect(body.errorMessage).to.equal("User has not applied to any jobs");
      done();
    });
  });
  ...
  it("Applicant has applied for any jobs", function (done){
    /**shou */
    let options = {phoneNumber: getAppliedUserApplied.phoneNumber}
    request(baseUrl + url, options, function(err, res, body) {
      expect(res.statusCode).to.equal(200);
      expect(body.errorMessage).to.equal("User hasn't applied for jobs");
      done();
    });
  });
...
describe("Get Applied-For Jobs", function() { 
  ...
  it("Success case", function (done){
    request(baseUrl + url, function(err, res, body) {
      expect(res.statusCode).to.equal(200);
      expect(body[0]).to.not.equal(undefined);
      done();
    });
  });
...
describe("User Is Able To Apply", function() { 
  ...
  it("User has already applied to this job", function (done){
    let options = {userID: canApplyAlreadyAppliedUser.userID, jobID: canApplyAlreadyAppliedJob.jobID}
    request(baseUrl + url, options, function(err, res, body) {
      expect(res.statusCode).to.equal(400);
      expect(body.errorMessage).to.equal("You have already applied to this job");
      done();
    });
  });
  ...
  it("User is the creator of this job", function (done){
    let options = {userID: canApplyIsCreatorUser.userID, jobID: canApplyIsCreatorJob.jobID}
    request(baseUrl + url, options, function(err, res, body) {
      expect(res.statusCode).to.equal(400);
      expect(body.errorMessage).to.equal("You can't apply to your own job");
      done();
    });
  });
  ...
  it("Correct application form", function (done){
    let options = {userID: canApplyCorrectUser.userID, jobID: canApplyCorrectJob.jobID}
    request(baseUrl + url, options, function(err, res, body) {
      expect(res.statusCode).to.equal(200);
      done();
    });
